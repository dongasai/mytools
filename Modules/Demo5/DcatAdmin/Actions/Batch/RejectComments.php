 ...
 * 批量审核拒绝评论
 */
class RejectComments extends BatchAction
{
    /**
     * 按钮标题 - 使用属性设置
     *
    ...
    public function handle()
    {
        ...
        try {
            ...
            \Modules\Demo5\Models\Demo5Comment::whereIn('id', $keys)
                ->update(['status' => CommentStatus::Rejected->value]);

            return $this->response()
                ->success('成功审核拒绝 ' . count($keys) . ' 条评论')
                ->refresh();
        } catch (\Exception $e) {
